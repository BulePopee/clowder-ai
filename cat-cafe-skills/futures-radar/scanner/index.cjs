...
function main() {
  ...
  // ── 99% percentile capping + min-max normalize ────────────
  function cappedNorm(values, capPct = 99) {
    if (values.length === 0) return values.map(() => 0);
    const cap = percentile(values, capPct);
    const capped = values.map(v => Math.min(v, cap));
    const min = Math.min(...capped);
    const max = Math.max(...capped);
    if (max === min) return capped.map(() => 0.5);
    return capped.map(v => (v - min) / (max - min));
  }

  const volPctVals = scored.map(s => s.indicators.volPercentile ?? 0);
  const atrPctVals = scored.map(s => s.indicators.atrPct ?? 0);
  const volMultVals = scored.map(s => s.indicators.volMultiplier ?? 1);
  const ret5dVals = scored.map(s => Math.abs(s.indicators.change5d ?? 0));

  const volPctNorm = cappedNorm(volPctVals);
  const atrPctNorm = cappedNorm(atrPctVals);
  const volMultNorm = cappedNorm(volMultVals);
  const ret5dNorm = cappedNorm(ret5dVals);

  // ── Weighted scoring ───────────────────────────────────────
  // Score = volPercentile × 0.35 + ATR%_norm × 0.25 + VolMult_norm × 0.20 + |5d_return|_norm × 0.20
  for (let i = 0; i < scored.length; i++) {
    scored[i].score = parseFloat((
      volPctNorm[i] * 0.35 +
      atrPctNorm[i] * 0.25 +
      volMultNorm[i] * 0.20 +
      ret5dNorm[i] * 0.20
    ...
    scored[i].scoreBreakdown = {
      volPercentile: parseFloat((volPctNorm[i] * 0.35).toFixed(4)),
      atrPct: parseFloat((atrPctNorm[i] * 0.25).toFixed(4)),
      volMultiplier: parseFloat((volMultNorm[i] * 0.20).toFixed(4)),
      absRet5d: parseFloat((ret5dNorm[i] * 0.20).toFixed(4))
    };
  }
  ...
  const output = {
    meta: {
      ...
      scoring: {
        formula: 'volPercentile×0.35 + ATR%_norm×0.25 + VolMult_norm×0.20 + |5d_return|_norm×0.20',
        normalization: '99% percentile capping + min-max',
