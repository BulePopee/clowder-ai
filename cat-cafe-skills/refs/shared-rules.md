...
> 版本号格式：`COORD/v{major}.{minor}`。major 升 = 不兼容的行为变更（球权语义重定义、路由规则改方向）；minor 升 = 新增规则/澄清/补示例（向后兼容）。
> 每次修改本文件或任何协作规则文件（`ball-reception-rule.md`、`serial-handoff-pattern.md` 等）时，commit message 必须含 `[COORD/vX.Y]` 标记变更性质。
> 新版本从下一个 F 级 feature kickoff 开始生效，已有 feature 在原版本下跑完不追溯升级。例外：feature 已到 merge 阶段时发现规则缺陷 → 记录为已知偏离，不进版本链。
...
**工具不可用证据链**：声明工具/能力不可用（WebSearch/WebFetch 等）必须附证据链——工具名、报错摘要、重试记录、是否尝试替代路径。0 次尝试就报错违反本契约（违反 P5）。正确流程：先试 → 失败 → 附证据 → 再声明不可用。
...
**协作规则变更标记**：修改以下任一文件时，commit message 必须含 `[COORD/vX.Y]` 标记变更性质（major 升级 / minor 增补）：
- `cat-cafe-skills/refs/shared-rules.md`
- `memory/ball-reception-rule.md`
- `memory/serial-handoff-pattern.md`
- 其他协作规则文件（由协调猫在 kickoff 时声明）

示例：`feat: 新增 fallback 层数检测协议 [COORD/v1.1]`（minor 升——新增规则，向后兼容）。版本号变更依据见本文件头部版本声明。
...
@ 是**球权转移**——行首 `@句柄` 触发对方的新调用。收到球后三选一：**接（我来做 X）、退（退给 @xxx）、升（需要铲屎官拍板）**。

**球权只有第一人称**：只能声明**自己**的球权，不能声明别人的。球权凭据只有 @ 或 hold_ball **动作本身**——没有动作，球权就没有转移。

**不变量**：
- 收到 @ → 行动、退回、或升级。发出 @ → 等待下次收球
- 无传球动作（@ / hold_ball）→ 禁止说"球在 X 手上"
- 对方 @ 了你又自称"在动" → push back + 紧跟接/退/升
- 收球后说"你等着" → 球权死锁，禁止。做不了就退回或升级
...
**SOP 链条**：review 完 → @ author / 修完 → @ reviewer / merge 完 → @ 愿景守护猫 / 分析完成 → @ 提问者

**没有第四种。** "到我这里结束了" ≠ "链条结束了"——先问"哪只猫能接"，再决定用哪种出口。`@铲屎官` 是硬条件出口，不是默认收尾口。
...
**非法**（反问式 ping — 把决定权塞回铲屎官）：`要不要 X？` / `落 spec 吗？` / `同意我就做`
**合法**（带立场 + 动作已执行或硬条件触发）：`已落 X（commit Y），反对来找我撤` / `请拍板 A vs B，推荐 A（硬条件-2），理由 X/Y/Z`

**识别标志**：能说"我决定 X"就自决。`@铲屎官` 是"我没资格单方面决定"的出口，不是"我不想决定"的出口。

### A2A 状态迁移

三类合法消息：
- **BLOCKED**：真的卡住，需要对方现在决策
- **REVIEW READY**：到了 review / 五件套边界
- **DONE**：任务结束，明确下一棒

接球后首个动作必须可观测——tool_call 开始执行 / 创建 task / 发 BLOCKED 附证据。纯文本"我接"会话结束 = 球掉地上（既不是状态迁移也不是执行动作），审计日志里不可验证。

接球后静默执行到下一个迁移点——中间产出留在代码里，不发。声明 = 执行：说"我进 merge gate"就在同一 turn 里做。发消息前自检：不是这三种 → 不发 → 继续做。

### §10.5 任务路由与子球执行（协调猫统一调度）

> 2026-05-09 铲屎官决策：不再由每只猫自行分类，改为协调猫（远远/阿比西尼亚猫）统一调度。

- 铲屎官 @某执行猫 → 单猫任务，该猫直接处理
- 铲屎官 @远远 或默认路由 → 远远拆解、匹配猫能力、分发子球、回收汇总闭环
- 协调角色与限制以 cat config 为准；工作流提示由 SystemPromptBuilder WORKFLOW_TRIGGERS 注入

**收到协调猫子球后：**
1. **直接执行** — 不重新分类、不走路由。子球 = 最终指令
2. **拒绝权** — 判断自己不是最佳人选 → push back 退给远远 + 建议更适合的猫
3. **三态回报** — 可做且已完成 / 不可做（附原因）/ 超时未完成（附卡点）
4. **子球不过户** — 不得转给其他猫。需协作 → 退回远远重分配

**协调猫汇报纪律**：汇报子球状态时必须显式区分：
- **已接球**：执行猫已回复确认，但尚无 tool_call / task 创建 / 中间产出（= 球可能在地上）
- **已开工**：有可观测执行痕迹（tool_call 调用记录 / task 状态变更 / 中间产出文件）
禁止以"已接球"包装"未开工"——铲屎官会把确认话术理解为任务已在进行。

### §10.6 协调猫 L1 边界块 + Gate-Before-Action

> 协调猫负责分配、归因、回收标准，不负责实现路径。发子球/汇总/给执行猫下轮动作时适用。

**边界块五格**：目标 → 接球猫 → 为什么是他（能力归因） → 回收证据 → 禁止展开到实现（不写文件路径、代码形状、实现步骤、测试手段）。

**Gate + 一票否决**：发球前自检是否出现 ①文件路径+改动动词（改/加/删/新增/调整） ②代码形状（类型签名/函数签名/伪代码/diff） ③执行指令（补单测/跑命令/改接口）→ 命中即打回。软信号：消息从”谁来做”滑成”怎么做”、语气从协调滑成定方案、替执行猫收敛技术路线 → 人工审查。

**判定原则**：协调猫可以定义问题、约束边界、指定验收；不能提供实现路径、代码形状、验证手段。

### §10.7 协作转交规范（先做再 @）

> 2026-05-08 实战发现：提前行首 @ 预约导致收球方无事可做，双方球权死锁。

**@ 是现在时的行动指令，不是将来时的预约通知。没做完不 @ 下家。**

| 场景 | 正确 | 错误 |
|------|------|------|
| 需要后续协作但没做完 | 正文提名不带 @，做完再行首 @ | 提前行首 @ → 球权死锁 |
| 收到提前 @ | Push Back："你还没做完，做完再 @我" | 说"等你做完叫我"→ 收球不动 |
| 确需并行 | `create_task` 建毛线球或 `hold_ball` | 提前 @ 当并行用 |

**小狸结束符原则**：一条消息的最后一个行首 @ 决定球权去向；目标猫不能立刻接手 → 不该发这个 @。
...
**@路由铁律（不可妥协）：**
- @句柄必须与上方「队友名册」表格中的 @mention 列**完全一致**（大小写不敏感）。叫错名字 = 球掉地上 = 消息送不到目标猫。
- 不确定某只猫的正确 @句柄时 → 先看队友名册，不凭记忆猜。名册是运行时动态生成的，比训练数据准确。
- 禁止使用 @全体成员、@all、@everyone 等不存在于名册中的句柄。


### §10.8 终局态 — 收束闭合协议（COORD/v1.1）

> 防"有产出但边际价值递减"型乒乓——报告完成后逐轮措辞抛光，每轮增量趋零。
> F167 L1 熔断覆盖无产出 chatter；本协议覆盖产出递减型往返。

**进入条件**（AND，缺一不可）：
1. 当前讨论已至少完成 ≥1 轮实质修改 + review 回应
2. 闭合声明附带可外部验证的工件引用（review comment 已解决 / checklist 已满 / PR 已合并 / 测试已绿），裸短语不触发
3. 使用闭合短语（任一）："可交付" / "不建议再扩" / "技术结论已闭合" / "决策权交回铲屎官"

**终局态后**：
- 禁止：同层复核（措辞抛光 / "再确认一下" / "要不要再补一轮"）
- 允许：铲屎官新指令 / 新的、具体的事实性异议（附外部引用）
- 纯措辞偏好不构成事实性异议

**回弹**：接收方发起同层复核 → 声明方只回闭合短语，不展开。确信有实质遗漏 → 附证据升级铲屎官。

**生命周期**：当前 session 有效；铲屎官发言立即解除；跨 session 自动解除。
...
> **Preflight 告警规则**（2026-05-09 铲屎官决策）：`cat-config.json` 和 `cat-template.json` 有 shared-state preflight 检测——任何未提交改动都会触发告警，阻塞工作流。**铲屎官决定：不修改任何会触发此告警的文件。** 即使是最小化 Edit 插入也可能触发。这两文件是 shared-state 的圣域——碰了 = 告警 = 工作流阻塞。需要改 → 先向铲屎官说明理由，获批后在 main 上改、改完立刻 commit+push。

| 层 | 机制 | 行为 |
|----|------|------|
| L1 | `.githooks/pre-commit` | 非 main 分支 commit → **硬拦** |
| L2 | `invoke-single-cat.ts` runtime preflight | unpushed → **硬拦**（停止调用）；uncommitted → warn |
| L2.5 | `.claude/hooks/shared-doc-push-guard.sh` | Claude 专属提醒：`cat-config.json` / `cat-template.json` uncommitted → warn（不拦截） |
