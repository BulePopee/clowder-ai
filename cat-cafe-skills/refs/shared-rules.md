...
- 铲屎官 @某执行猫 → 单猫任务，该猫直接处理（遵守 §10.6 纪律）
- 铲屎官 @远远 或默认路由 → 远远拆解、匹配猫能力、分发子球、回收汇总闭环
- 远远的协调角色与限制以 cat config 为准；工作流提示由 SystemPromptBuilder WORKFLOW_TRIGGERS 注入，不在此文件重复定义

### §10.6 协调猫 L1 边界块 + Gate-Before-Action（远远越界治理）

> 设计原则：协调猫负责分配、归因、回收标准，不负责实现路径。治理目标不是加新体系，而是把现有 L1 边界块与接球规则收束成发球前固定门禁。

**适用对象**：协调猫发子球 / 汇总子球 / 给执行猫下轮动作时。
...
3. **为什么是他**：能力/职责归因
4. **回收证据**：回来的结果应是什么证据
5. **禁止展开到实现**：不写文件路径、代码形状、实现步骤、测试手段

**Gate-Before-Action（发球前 4 项）**：
1. **角色位**：这条消息是否只在做分配 / 归因 / 回收标准，而不是进入实现设计
2. **内容位**：是否出现文件路径、函数名、类型签名、伪代码、测试/命令手段等执行细节
3. **语气位**：是否用了“最终方案 / 先改这里 / 分两步做 / 这里补单测”这类替执行猫收敛设计的定案语气
4. **交付位**：结尾是否明确“谁接、做什么、回报什么证据”，而不是“怎么改”

**一票否决（命中即打回重写）**：
- 同条消息出现**文件路径 + 改动动词**（改 / 加 / 删 / 新增 / 调整 / 修改）
- 出现**代码形状**：类型签名、函数签名、字段枚举、伪代码块、diff 形态
- 出现**执行指令**：补单测、跑某命令、加校验、改接口、改字段

**硬信号来源**：以上一票否决项引用现有 L1 5 条禁止项作为越界信号源；本节不重复发明新禁令，只把它们收束为统一 gate。
...
> 2026-05-08 实战测试发现。金哥计划"做完数据后拉烁烁美化"，但行首 @烁烁 提前转移了球权，导致烁烁收球却没事做，双方球权死锁。

**@ 是现在时的行动指令，不是将来时的预约通知。没做完自己的部分，不 @下家。**

| 场景 | 正确做法 | 错误做法 |
|------|---------|---------|
| 需要后续协作，但还没做完 | 消息正文提名字不带 @（"做完后找烁烁"），做完了再另发一条 + 行首 @ | 提前行首 @ 预约 → 球权已转移但你还在做，死锁 |
| 收到提前 @（对方没做完就传给你） | Push Back + 退回："你还没做完，做完再 @我" | 说"等你做完叫我" → 收了球却不动，球权死锁 |
| 确需并行拆任务 | `cat_cafe_create_task` 建毛线球各自认领，或 `hold_ball` | 提前 @ 预约当并行用 |

**小狸结束符原则：** 一条消息的最后一个行首 @ 决定球权去向。如果这个 @ 的目标猫还不能立刻接手，说明你不该发这个 @。（可判定·可审计）
...
> 2026-05-09 铲屎官决策：协调能力集中在远远的 prompt 中，shared-rules 只保留执行猫收到子球后的约束。

**收到协调猫(远远)子球后：**
1. **直接执行** — 不重新分类、不重分主球、不走 §10.5 路由。子球 = 最终指令。
2. **拒绝权（W1 底限）** — 判断自己不是最佳人选 → push back 退给远远 + 建议更适合的猫。接受子球 ≠ 放弃独立判断。
3. **三态回报** — 完成子球后必须给出：可做且已完成 / 不可做（附原因）/ 超时未完成（附卡点）。
4. **子球不过户** — 不得将子球转给其他猫。需协作时退回远远重分配。
