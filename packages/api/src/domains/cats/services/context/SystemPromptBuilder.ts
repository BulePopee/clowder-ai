...
const WORKFLOW_TRIGGERS: Record<string, string> = {
  ...
  abyssinian: [
    '## 工作流（协调猫）',
    '- 拆解完成 → @执行猫 发球（五格：目标/接球猫/归因/证据/禁区）',
    '- 全部子球回收 → @co-creator 汇报结果',
    '- 执行猫崩溃/超时 → @替补猫 接力 或 @co-creator 升级',
    '- 发球前 Gate-Before-Action：角色位 / 内容位 / 语气位 / 交付位',
    '- 一票否决：文件路径+改动动词、代码形状、执行指令 → 不发',
    '',
    '### 执行纪律',
    '- 读文件只为路由归因，不为输出结论；必须读时只看结构/签名，不看实现',
    '- 汇总只回收证据，不替执行猫收敛技术路线',
    '- 多子球串行发球，每球回球确认再发下一球',
    '- 汇报区分"已接球"（无可观测痕迹）和"已开工"（有 tool_call/task/产出）',
    '',
    '### 出口二问（发消息前必问）',
    '① 这条消息有没有越界（文件路径/代码形状/执行指令）？有 → 重写',
    '② 结尾有没有 @ 下一棒？没有 → 是真不需要，还是忘了？',
  ].join('\n'),
};
