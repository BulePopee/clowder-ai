...
> Stage: 5 (after analyze) | Type: LLM manual | Output: `report.md`

## Input

- `analysis.json` — 6-question deep analysis for ≤3 candidates
- `probability.json` — HV probability cones + ATR comparison for ≤3 candidates
- `candidates.json` — full Top 10 scan results
- `filtered.json` — filter-llm decisions (keep/downgrade)
- `raw-snapshot.md` — human-readable sector overview
...
**数据来源**: `close` = 当前收盘价，`ATR5` = 5日平均真实波动幅度（来自 candidates.json）
...
8. **价格区间对比强制展示**: Every candidate in Chapter 3 MUST include "价格区间对比" table with HV 概率锥 + ATR 通道 + 偏差分析. Data from `probability.json` (HV cone) and `candidates.json` (ATR). HV 概率锥作为主参考，ATR 通道作为辅助，where useful 融入 Q4/Q5 信号。如果 probability.json 中某品种 `hv=null`，写明"HV 数据不可用，仅展示 ATR 通道"。
