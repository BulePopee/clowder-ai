...
const artifacts = [
  ...
  },
  {
    ...
    path: '{runDir}/websearch-results.json',
    stage: 'websearch-fill',
    required: true,
    requiredForReport: false,
    note: 'WebSearch pending fill artifact — template generated by websearch-fill.cjs --mode template, filled by LLM, reflowed by --mode fill',
    producedBy: 'collector/websearch-fill.cjs',
    consumedBy: ['validate-source-contract-post']
  },
  {
    id: 'portfolio-json',
...
const stages = [
  ...
  {
    id: 'websearch-fill',
    label: 'WebSearch 补采 + 回流 (Manual)',
    auto: false,
    dependsOn: ['validate-source-contract', 'collect'],
    inputs: ['raw-json', 'provenance-json'],
    outputs: ['websearch-results'],
    validators: ['validate-source-contract-post'],
    failurePolicy: 'warn',
    manualInstruction: 'LLM: read websearch-results.json template, run WebSearch for each pending item, fill value/date/sourceUrl/sourceTitle. Then: node collector/websearch-fill.cjs --runId {runId} --mode fill. ContractDecision/writePolicy are recalculated by the script — do NOT fill those fields.',
    note: 'LLM: run WebSearch for each pending item in websearch-results.json. Fill only: value, date, sourceUrl, sourceTitle, notes. Then node collector/websearch-fill.cjs --runId {runId} --mode fill. Script recalculates contract enforcement.'
  },
  {
    id: 'validate-source-contract-post',
    label: '验证数据源契约 (post-reflow)',
    auto: true,
    dependsOn: ['websearch-fill'],
    inputs: ['websearch-results', 'raw-json', 'provenance-json'],
    outputs: [],
    validators: [],
    failurePolicy: 'hard_fail',
    rebuildCommand: 'node collector/validate-source-contract.cjs --runId {runId}',
    script: 'collector/validate-source-contract.cjs',
    args: (runId) => ['--runId', runId],
    note: 'Re-runs source contract validation after websearch reflow. Catches: blocked items in raw, missing provenance, forbidden values.'
  },
  {
    id: 'portfolio',
    label: '采集 (组合持仓)',
  ...
  // ── Stage 3: Compute ──
  {
    ...
    label: '派生计算',
    auto: true,
    dependsOn: ['validate-source-contract-post', 'collect'],
  ...
  // ── Final Gate ──
  {
    ...
    label: '全链路一致性校验',
    auto: true,
    dependsOn: ['validate-indicator-contract', 'validate-source-contract', 'validate-source-contract-post', 'validate-reasoning', 'validate-temporal-diff', 'validate-feedback', 'validate-report', 'publish-current'],
